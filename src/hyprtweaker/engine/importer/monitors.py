...
def _reserved(values: list[str], notes: _Notes) -> dict[str, int]:
...
def _scale(raw: str, notes: _Notes) -> Any:
    stripped = raw.strip()
...
def _mode(raw: str, notes: _Notes) -> str:
    stripped = raw.strip()
    if not stripped:
        return "preferred"
...
def _typed(key: str, raw: str, notes: _Notes) -> Any:
    if key == "sdr_eotf":
        stripped = raw.strip()
...
def map_monitor(
    ...
    replaces wholesale.
    """
    notes = _Notes(report, origin, f"monitor = {value}")
    parts = [part.strip() for part in value.split(",")]
    output = parts[0]
    rest = parts[1:]
    if not rest:
        notes.add(LossCode.UNSUPPORTED_KEYWORD, "monitor line has only a name")
        return None
    ...
    while index < len(trailing):
        ...
        if index + 1 >= len(trailing):
            notes.add(LossCode.UNSUPPORTED_KEYWORD, f"monitor setting {key!r} has no value")
            break
...
def map_monitor_block(
    ...
    notes = _Notes(report, origin, "monitorv2 { ... }")
