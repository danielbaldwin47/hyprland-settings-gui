...
| **Curation flag** | A demand for hand curation the generator records on an Option it can detect a defect in but not fix — sentinel default, map-less small int, prose-only value list, source-only bounds, free-text string that is really a picker. Each flag names the Overlay field that answers it; the completeness test fails the build on any flag left unanswered (ADR-0011). |
| **Sentinel** | A default that means "no value": `[[EMPTY]]`, `[[Auto]]`, or `-1`. The 22 unambiguous ones resolve to a null default automatically; a numeric `-1` is flagged instead, because it means "unset" for one Option and "random" for another. Rendering a sentinel as data is the defect class prototype #8 found most damaging. |
