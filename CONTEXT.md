...
| **Preset** | A named bundle of Option values saved by Capture scope; look-and-feel only — never binds/rules/monitors (Preset ≠ Profile; whole-setup carry-over is Export, ADR-0009). Lives at `presets/<slug>.json` in the App dir; applied via a normal Apply transaction (ADR-0014). |
